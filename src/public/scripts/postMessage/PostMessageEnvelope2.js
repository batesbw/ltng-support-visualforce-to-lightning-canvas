...
function PostMessageEnvelope(pageName, messageType, isSuccessful, payload) {
...
**/
PostMessageEnvelope.prototype.dispatch = function dispatch(targetWindow, targetDomain) {
  const myTargetDomain = (!targetDomain ? targetDomain : '*');
  const dataStr = '';
  const payload = {
    sender: this.sender,
 ...
 *  @return boolean - whether the event was sent from that page
 *    in a case insensitive manner (true) or not (false)
 *  <p>TypeScript: matchesPage( pageName:String ):boolean</p>
**/
PostMessageEnvelope.prototype.matchesPage = function matchesPage(pageName) {
 ...
 *  @return boolean - whether the message matches
 *    in a case insensitive manner (true) or not (false)
 *  <p>TypeScript: matchesMessageType( messageType:String ):boolean</p>
**/
PostMessageEnvelope.prototype.matchesMessageType = function matchesMessageType(messageType) {
 ...
 *  @param pageName (String)
 *  @param messageType (String)
 *  @return boolean - whether the pageName and the messageType
 *    both match in a case insensitive manner.
 *  <p>TypeScript: matchesPageMessage(
 *    pageName:String,
 *    messageType:String
 *  ):boolean</p>
**/
PostMessageEnvelope.prototype.matchesPageMessage = function matchesPageMessage(
  pageName,
  messageType,
) {
...
**/
PostMessageEnvelope.prototype.getMessageOrigin = function getMessageOrigin(evt) {
...
**/
PostMessageEnvelope.prototype.parse = function parse(evt) {
  const results = null;
  try {
    if (!evt.data.sender) {
      throw (new Error('not a LNE_PostMessage'));
    }
