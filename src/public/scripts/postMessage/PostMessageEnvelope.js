...
class PostMessageEnvelope {
  ...
  **/
  dispatch(targetWindow, targetDomain) {
    ...
    targetWindow.postMessage(payload, myTargetDomain);
  }
  
  /**
   *  Whether the event was sent from a given page.
   *  @param pageName (String)
   *  @return boolean - whether the event was sent
   *    from that page in a case insensitive manner (true) or not (false)
  ...
  **/
   ...
   *    pageName:String, messageType:String
   *  ):boolean</p>
  **/
  ...
  **/
  static parse(evt) {
    ...
    try {
      ...
      results = new PostMessageEnvelope(
        evt.data.sender, evt.data.messageType,
        evt.data.isSuccessful, evt.data.data,
      );
    } catch (err) {
